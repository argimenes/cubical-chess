...
export class LatticeView {
  ...
  constructor() {
    ...
    this.root.add(this.grid,this.structural,this.edge,this.local); this.setMode('full');
  }
  setMode(mode: LatticeMode): void {
    this.mode=mode; this.grid.visible=mode!=='structural'; this.structural.visible=mode==='structural'; this.local.visible=mode==='adaptive';
    ...
    this.homes.forEach(grid=>{ grid.visible=mode!=='structural'; (grid.material as THREE.Material).opacity=mode==='adaptive'?0.035:0.2; });
  }
  ...
  metrics(){ return {mode:this.mode, localCells:this.mode==='adaptive'?[...this.localCells]:[], localSegments:this.mode==='adaptive'?this.localCells.length*24:0}; }
