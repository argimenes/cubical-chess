...
Select a piece to see its full movement field. Hover a destination or focus its button to emphasize that marker and inspect one path: a continuous ray for sliders, a dashed elbow for Knights. The inspector explains displacement, capture and prospective check without changing the game. Reflective, metallic gold markers indicate legal destinations; larger, warmer gold markers indicate captures. The material uses a small shared reflection map so it remains legible in every theme. Hover or focus a gold destination to reveal a smaller metallic **silver** field for that same piece from the prospective position; other gold destinations dim. Silver is a read-only what-if view assuming the opponent’s position stays unchanged. It accounts for the first move’s capture, vacated origin and king safety; promotion previews include all promotion choices. A move ending the game has no following field. Click or press Enter on the gold destination to make only the first move. On touchscreens, tap a destination to inspect it, then tap it again to commit.
