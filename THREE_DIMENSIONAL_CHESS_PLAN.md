...
**Crystal art-direction follow-up:** the earlier bounded optical pass was visually too subtle. The approved next experiment deliberately overshoots: a continuous cubical cut crystal with connected bevels, internal cleavage facets, localized rainbow bands, caustic concentrations and glints. An initial arrangement of detached facets read as an exploded crystal; the geometry now forms one intact volume. An optional 8×8×8 cubical-inclusions comparison retains the same outer envelope and highlights only a few cells at each viewing angle. Crystal opens with the combined study, inclusions off, and retains independent component switches; Diagnostic remains the application default and Luminous retains its approved appearance. Sparse geometry replaces the expensive six-sample screen pass. Keep rules, picking, piece silhouettes and movement fields protected. The [exaggerated study report](docs/CRYSTAL_EXAGGERATED_STUDY.md) records comparisons and performance before reducing intensity or developing other themes.

**Limited architecture spike:** retain Diagnostic as the default and add one restrained Abstract/Luminous study over the same semantic scene. Themes own environment, volume styling, lighting, piece visuals/materials, effects and animation; common picking, gold movement fields and authoritative rules stay independent. A camera-only director supports explicit smooth focus and interruptible inspection orbit. Saved-game replay, cinematic replay and attract mode will use a separate replay session/scheduler that cannot overwrite the active game. The [theme/camera report](docs/THEME_CAMERA_SPIKE.md) maps Crystal, Luminous, Deep Space and Aquarium to this contract and records performance/legibility gates before further polish.

Use the concept image's dark background, central blue crystal volume, blue/cyan versus warm/red holographic armies, fine grid, and compact side panels as a design starting point. Render the 512-cell lattice with batched/instanced elements where repeated geometry warrants it; [InstancedMesh](https://threejs.org/docs/pages/InstancedMesh.html) can reduce draw calls. For the eventual playing intensity, tune refraction, bloom and chromatic edges after the deliberately exaggerated Crystal study establishes an upper aesthetic bound; profile on representative hardware. The silhouette of pieces and destination markers should remain legible from common camera angles.
