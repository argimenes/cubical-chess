...
**Current bounded visual study — electric Luminous:** following human review, stop developing a strongly crystalline playing volume. The lattice is a thin, geometrically accurate navigation instrument with theme-independent Full, Structural and Adaptive settings. Concentrate visual identity in the environment, omnidirectional piece forms, local light/particles, move/capture effects and camera choreography. Luminous now explores a near-black starfield and sapphire/cyan versus ruby/red faceted pieces with localized electrical activity. Gold/amber interactions and authoritative rules remain common. The [Luminous electric report](docs/LUMINOUS_ELECTRIC_STUDY.md) records recognition sheets, spatial legibility and performance before final polish.

**Crystal reference:** retain the completed intact-volume and cubical-inclusions experiment and [its report](docs/CRYSTAL_EXAGGERATED_STUDY.md). Do not increase whole-volume refraction, dispersion or caustics. Diagnostic stays available as a clean reference.

**Architecture:** themes own environment, lighting, piece visuals/materials, local effects and animation. The common view owns lattice display, picking, destination fields and inspection aids. The camera-only director supports explicit focus, orbit and close inspection, immediately interruptible by input. Close inspection can enter the volume without an enclosing Luminous surface. Saved-game/cinematic replay and attract mode require a separate replay session/scheduler that cannot overwrite the active game; they are not implemented by this study.
