...
| 3. Spatial UI and history | Design intrinsically three-dimensional piece symbols, then add holographic treatment, slice/plane emphasis, coordinate labels, selected-piece explanation, promotion choice, move log, turn and game status; add responsive desktop/tablet panels and local game records. | Piece types remain identifiable from front, rear, sides, above, below and oblique views; new players can interpret 3D destinations; reload restores an active game and the archive can open a saved game. |
...
Default assistance should show legal destinations for the selected piece. Trajectories, enemy threat field, and slice emphasis are separate toggles. Slider guides end at blockers; knight guides must look like jumps. Threats come from the rules engine's attacked-cell map, including pawn capture cells, not from the geometry of displayed legal moves. A ghost preview can show the consequences of a hovered legal move without committing it. Layer isolation must help inspect the cube, while the complete cube remains the default playable view.
